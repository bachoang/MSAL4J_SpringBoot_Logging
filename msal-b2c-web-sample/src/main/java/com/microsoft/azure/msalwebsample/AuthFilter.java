...
public class AuthFilter implements Filter {
    ...
    private void processAuthenticationCodeRedirect(HttpServletRequest httpRequest, String currentUri, String fullUrl)
        ...
        System.out.printf("%s%n", "in processAuthenticationCodeRedirect");

        Map<String, List<String>> params = new HashMap<>();
        for (String key : httpRequest.getParameterMap().keySet()) {
            System.out.printf("%s%n",key + "=" + Collections.singletonList(httpRequest.getParameterMap().get(key)[0]));
            params.put(key, Collections.singletonList(httpRequest.getParameterMap().get(key)[0]));
        }
        // validate that state in response equals to state in request
        System.out.printf("%s%n", "about to call validateState");
    ...
    void sendAuthRedirect(String authoriy, HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
        ...
        System.out.printf("%s%n", "in sendAuthRedirect");
        System.out.printf("%s%n", "state = " + state);
    ...
    private void validateState(String cookieValue, String state) throws Exception {
        System.out.printf("%s%n", "in validateState");
        System.out.printf("%s%n", "cookieValue: "+ cookieValue);
        System.out.printf("%s%n", "state: " + state);
    ...
    private String getRedirectUrl(String authority, String claims, String state, String nonce)
        ...
        // Debug.println("in get redirecturl");
        // println("in redirect");
